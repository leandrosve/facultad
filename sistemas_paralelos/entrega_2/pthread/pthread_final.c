...
int main(int argc, char *argv[])
{
    ...
    pthread_mutex_init(&escalar_lock, NULL);
    pthread_mutex_init(&count_bt_lock, NULL);

    sem_init(&bt_sem, 0, 0);
    sem_init(&escalar_sem, 0, 0);
...
void *thread_process(void *ptr)
{
    ...
    // ======= Parte 1: Armar B transpuesta =======
    // Lo hacemos antes asi no tenemos que usarla enseguida
    int startb = id * ROWS_PER_THEAD;
    int endb = startb + ROWS_PER_THEAD - 1;
    ...
    pthread_mutex_lock(&escalar_lock);
    if (maxALocal > maxA)
        maxA = maxALocal;
    ...
    if (count_escalar == T)
    {
        ...
        for (i = 0; i < T; i++)
        {
            sem_post(&escalar_sem);
        }
    }
    pthread_mutex_unlock(&escalar_lock);
    ...
    // Garantizar que el escalar esta calculado
    sem_wait(&escalar_sem);
    ...
    // No necesitan esperar, porque utilizan las posiciones de AxB que calculron ellos mismos
