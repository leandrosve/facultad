...
void *thread_process(void *ptr)
{
   ...
   // printf("\nHilo: %d: [%d, %d]\n", id, inferior, superior);
    ...
    pthread_mutex_lock(&maxA_lock);
    if (maxALocal > maxA)
        maxA = maxALocal;
    pthread_mutex_unlock(&maxA_lock);

    pthread_mutex_lock(&maxB_lock);
    if (maxBLocal > maxB)
        maxB = maxBLocal;
    pthread_mutex_unlock(&maxB_lock);

    pthread_mutex_lock(&minA_lock);
    if (minALocal < minA)
        minA = minALocal;
    pthread_mutex_unlock(&minA_lock);

    pthread_mutex_lock(&minB_lock);
    if (minBLocal < minB)
        minB = minBLocal;
    pthread_mutex_unlock(&minB_lock);
