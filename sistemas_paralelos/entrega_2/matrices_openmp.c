...
int main(int argc, char *argv[])
{
  ...
  int tid;
#pragma omp parallel for private(i) reduction(min : minA) reduction(max : maxA) reduction(+ : promA)
  for (i = 0; i < NxN; i++)
  {
    double aux = A[i];
...
#pragma omp parallel for private(i) reduction(min : minB) reduction(max : maxB) reduction(+ : promB)
  for (i = 0; i < NxN; i++)
  {
    double aux = B[i];
